...
def load_model_vocoder(
    ...
    resolved_vocoder_ckpt = str(resolve_checkpoint_path(
        vocoder_ckpt if vocoder_ckpt is not None else args.vocoder.ckpt,
        asset_root=asset_root,
        model_dir=model_dir
    ))
    args['vocoder']['ckpt'] = resolved_vocoder_ckpt
    print(' [Vocoder] ' + resolved_vocoder_ckpt)

    # load vocoder
    vocoder = Vocoder(args.vocoder.type, resolved_vocoder_ckpt, device=device)
...
def resolve_checkpoint_path(path, asset_root=None, model_dir=None):
    path = os.fspath(path)
    candidates = []
    if os.path.isabs(path):
        ...
        if asset_root is not None:
            if isinstance(asset_root, (list, tuple)):
                asset_roots.extend(asset_root)
            else:
        ...
        if model_dir is not None:
            candidates.append(os.path.join(model_dir, path))
        candidates.append(path)
    for candidate in candidates:
        if os.path.exists(candidate):
            return os.path.abspath(candidate)
        ckpt_candidate = candidate + '.ckpt'
        if os.path.exists(ckpt_candidate):
            return os.path.abspath(ckpt_candidate)
...
class NsfHifiGAN(torch.nn.Module):
    def __init__(self, model_path, device=None):
        ...
        if device is None:
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.device = device
        self.model_path = resolve_checkpoint_path(model_path)
        self.model = None
        self.h = load_config(self.model_path)
        self.stft = STFT(
